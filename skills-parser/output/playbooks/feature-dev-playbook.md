...
3. **Generate SDD specification**
   - Produce a Software Design Document covering:
     ...
     - **Security design**: Authentication requirements, authorization rules (RBAC/ABAC), input validation rules, data encryption requirements per NIST 800-53 and STIG.
...
5. **Implement the feature**
   - Select the execution spoke:
     ...
     - **Devin CLI**: For UI components, complex business logic requiring human judgment, and features touching classified data handling.
   ...
   - Apply security controls during implementation, not after:
     - Input validation on all user-facing parameters (STIG V-220631).
     - Parameterized queries for all database access (STIG V-220632).
     - Structured audit logging for security-relevant actions (STIG V-220635).
     - Error handling with generic user messages and detailed internal logs (STIG V-220641).
...
6. **Run guardrail checks**
   - Execute the full DevinClaw guardrail suite:
     ...
     - **HG-010**: NIST 800-53 controls validated.
...
- If the feature requires a new database table, always include indexes for columns that will be used in WHERE clauses, JOIN conditions, or ORDER BY expressions. The the enterprise's PostgreSQL instances handle millions of records and index-free queries cause cascading performance issues.
...
- Do not write raw SQL queries with string concatenation. All database queries must use parameterized queries or an ORM's query builder to prevent SQL injection (STIG V-220632).
- Do not generate or store test data containing real PII, PHI, or sensitive enterprise perational data. All test data must be synthetic.
