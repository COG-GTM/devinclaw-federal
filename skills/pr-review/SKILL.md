...
This skill performs comprehensive automated code review on pull requests using Devin Review as the primary execution spoke. It goes beyond standard linting and formatting to provide deep semantic review: detecting bugs, security vulnerabilities, architectural drift, performance anti-patterns, and federal compliance violations. The skill integrates Devin Review's three core capabilities — auto-review for intelligent analysis, bug catcher for defect detection, and auto-fix for automated remediation — into a unified review pipeline that enforces DevinClaw guardrails on every PR.
...
- **Review scope** (optional): Focus area for the review. Options:
  ...
  - `security`: Security-focused review (STIG, NIST, OWASP).
...
- **Compliance frameworks** (optional): Which compliance frameworks to validate against. Default: all applicable (STIG, NIST 800-53, FedRAMP).
...
2. **Generate review specification (SDD)**
   - Produce a review specification document that defines:
     ...
     - **Risk assessment**: Classification of the PR by risk level:
     ...
     - **Applicable compliance checks**: Which STIG IDs, NIST controls, and OWASP categories apply to the changed code.
...
4. **Execute security compliance scan**
   - Run the STIG scanner via `stig-scanner-mcp` against all changed files:
     - **V-220631 Input Validation**: Verify all user inputs are validated with whitelist patterns.
     - **V-220632 Input Sanitization**: Verify parameterized queries for all database access. Flag any string concatenation in SQL.
     - **V-220633 Encryption at Rest**: Verify sensitive data fields use appropriate encryption.
     - **V-220634 Encryption in Transit**: Verify TLS configuration for any new network communication.
     - **V-220635 Audit Logging**: Verify security events are logged in structured JSON format.
     - **V-220641 Error Handling**: Verify error responses do not leak stack traces, internal paths, or sensitive data.
   - Run NIST 800-53 control validation via `nist-controls-mcp`:
     - Map each changed file to applicable NIST control families (AC, AU, IA, SC, SI).
   ...
   - Collect all security findings with STIG IDs, NIST control references, and CVE identifiers.
...
6. **Validate guardrail compliance**
   - Check all DevinClaw hard gates against the PR:
     ...
     - **HG-002 STIG Clean**: Verify zero High findings from step 4.
     ...
     - **HG-010 NIST Controls**: Verify applicable NIST controls are implemented.
...
8. **Compose and post review summary**
   - Compile all findings into a structured review summary posted as a PR comment:
     ...
     - **Security findings**: Listed with STIG IDs and NIST control references.
...
- **Review persistence**: All review findings, verdicts, and audit data are stored in the OpenClaw ledger for minimum 1 year per FAR 4.703.
...
- Do not skip the security compliance scan. Every PR must be scanned for STIG, NIST, and dependency vulnerabilities regardless of the change size.
...
- Do not disable or reduce the severity of STIG findings. STIG severity levels are defined by DISA and cannot be overridden by the review tool.
