---
name: security-scan
description: Scan codebases for STIG, NIST 800-53, and FedRAMP compliance violations. Use this skill when performing federal security audits on enterprise applications, when validating compliance posture before ATO submissions, or when scanning for vulnerabilities and security misconfigurations in modernized code.
---

# Federal Security Compliance Scanning

## Overview

This skill performs comprehensive security compliance scanning of codebases against DISA STIG benchmarks, NIST 800-53 control families, and FedRAMP baselines. It connects to the SonarQube MCP server for static analysis, runs the STIG scanner MCP for SCAP benchmark validation, checks NIST 800-53 controls through the nist-controls-mcp, and validates Zero Trust architecture principles. When violations are found, the skill generates SDD remediation specifications, TDD test cases for security fixes, and spawns Devin sessions to auto-remediate findings at scale.

Enterprises operate critical national infrastructure under federal oversight. All enterprise applications must maintain Authorization to Operate (ATO) through continuous compliance with FISMA, FedRAMP, and DISA STIG requirements. Safety-critical systems in the mission-critical systems  are additionally subject to DO-178C (Software Considerations in Airborne Systems and Equipment Certification) and DO-326A (Airworthiness Security Process Specification). A single unmitigated High finding on a STIG scan can block an ATO renewal, grounding modernization efforts. This skill automates the scan-remediate-validate cycle to maintain continuous compliance posture.
...
|-------|----------------|-------------|
| STIG Scan | `stig_scan.md` | `stig_scan.json` |
| NIST 800-53 Assessment | `nist_assessment.md` | `nist_assessment.json` |
| FedRAMP Validation | `fedramp_validation.md` | `fedramp_validation.json` |
...
- **Human approval required for**: CAT I finding risk acceptance, ATO package submission, safety-critical code changes (DO-178C DAL A-C), POA&M entry creation.
