...
- **Certificate generation** (optional): Whether to generate a formal SDLC completion certificate with digital signature for federal audit packages. Default is true.
...
- **Compliance framework** (optional): Which compliance framework to validate against beyond the baseline DevinClaw checklist (e.g., NIST 800-53, DISA STIG, FedRAMP Moderate, FedRAMP High). Default is all applicable frameworks.
...
- **Human approval required for**: ATO evidence package compilation, cross-boundary session validation, certificate issuance for safety-critical systems.
